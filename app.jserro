...
async function buildPublicCharts(){
  ...
  const chamadosHoje = rows.filter(r => r.dataEncaminhamento === hoje);

  const totalChamados = rows.length;
  const totalHoje = chamadosHoje.length;

  const elTotalChamados = document.getElementById('kpiTotalChamados');
  const elChamadosHoje = document.getElementById('kpiChamadosHoje');

  if(elTotalChamados){
    elTotalChamados.textContent = `Total de Chamados: ${totalChamados}`;
  }

  if(elChamadosHoje){
    elChamadosHoje.textContent = `Chamados Hoje: ${totalHoje}`;
  }
